...
class NAApiClient
{
    ...
    {
        $this->token_file_path = \Yii::getAlias('@runtime') . $this->token_file_name;
        if (file_exists($this->token_file_path)) {
            $this->refresh_token = file_get_contents($this->token_file_path);
        } else {
    ...
    private function connect()
    {
        ...
        if (isset($jsonDatas['refresh_token'])) {
            file_put_contents($this->token_file_path, $jsonDatas['refresh_token']);
            $this->refresh_token = $jsonDatas['refresh_token'];
            $this->access_token = $jsonDatas['access_token'];
            return true;
        }
    ...
    public function api($path, $method = 'GET', $params = array(), $secure = false)
    {
        ...
        $res = $this->makeOAuth2Request(
            $this->getUri($path, [], $secure),
            $method,
            $params
        );
        if (isset($res["body"])) {
     ...
     */
    protected function getUri($path = '', $params = [], $secure = false)
    {
